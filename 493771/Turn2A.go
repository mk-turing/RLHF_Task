...
func tierRateLimit(handler http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		key := fmt.Sprintf("rate-limit:%s:%s", r.RemoteAddr, tier)
		...
		if count > tierLimit {
			...
			http.Error(w, fmt.Sprintf("Rate limit for %s tier exceeded: %d requests in %s", tier, tierLimit, window.String()), http.StatusTooManyRequests)
			return
		}
...
func main() {
	http.HandleFunc("/", tierRateLimit(http.HandlerFunc(handler)).ServeHTTP)
