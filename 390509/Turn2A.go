...
func main() {
	...
	v1.HandleFunc("/posts", postController.CreatePost).Methods("POST")
	v1.HandleFunc("/posts", postController.GetAllPosts).Methods("GET")
	v1.HandleFunc("/posts/{id}", postController.GetPostById).Methods("GET")
	v1.HandleFunc("/posts/{id}", postController.UpdatePost).Methods("PUT")
	v1.HandleFunc("/posts/{id}", postController.DeletePost).Methods("DELETE")

	// Version v2 (potential future version with different logic or features)
	v2 := r.PathPrefix("/v2").Subrouter()
	// Here you could implement v2 specific handlers, e.g., with different data formats or business logic
	v2.HandleFunc("/posts", postController.CreatePost).Methods("POST")
	v2.HandleFunc("/posts", postController.GetAllPosts).Methods("GET")
	v2.HandleFunc("/posts/{id}", postController.GetPostById).Methods("GET")
	v2.HandleFunc("/posts/{id}", postController.UpdatePost).Methods("PUT")
	v2.HandleFunc("/posts/{id}", postController.DeletePost).Methods("DELETE")
