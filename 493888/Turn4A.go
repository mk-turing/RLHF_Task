...
func updateDocument(change DocumentChange, username string, role string) error {
	...
	notification := &Notification{
		...
		Message:   fmt.Sprintf("Document updated by %s", username),
		Timestamp: time.Now(),
		User:      username,
	}
	...
	_, err = redisClient.Publish(context.Background(), "notifications", notificationBytes).Result()
	if err != nil {
...
func handleConnection(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		username := "user1"
		err = updateDocument(change, username, "editor")
		if err != nil {
...
func sendNotifications(conn *websocket.Conn) {
	pubSub := redisClient.Subscribe(context.Background(), "notifications")
	defer pubSub.Close()

	for message := range pubSub.Channel() {
		notificationBytes := message.Payload

		var notification Notification
		err := json.Unmarshal([]byte(notificationBytes), &notification)
		if err != nil {
		...
		if err != nil {
			log.Printf("Error marshalling notification to JSON: %v", err)
			continue
		}
